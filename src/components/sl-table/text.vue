<template>
  <view
    ...
    :class="[tableHeight != 'auto' ? 'fix-height' : '', celCenter ? 'td-center' : '']"
  >
    <view class="table_box_big" :style="{ height: tableHeight }">
      <view class="table_box">
        ...
        <view class="div-table div-table-head">
          <view class="thead">
            <view class="tr">
              ...
              <view
                class="td"
                :style="{ width: countHeadColspanWidth(item, index), height: thTdHeight + 'px' }"
                      ...
                      <view
                        ...
                        :class="[
                          item.cellClassName && item.cellClassName[tdItem.key]
                            ...
                            : '',
                          spanMethod(item, tdItem, index, tdItemIndex)['rowspan'] == 0
                            ? 'empty-cells-for-rowspan'
                            : '',
                          spanMethod(item, tdItem, index, tdItemIndex)['colspan'] == 0
                            ? 'empty-cells-for-celspan'
                            : '',
                          spanMethod(item, tdItem, index, tdItemIndex)['rowspan'] > 1
                            ? 'rowspan'
                            : '',
                          spanMethod(item, tdItem, index, tdItemIndex)['colspan'] > 1
                            ? 'colspan'
                            : '',
                        ]"
                        :style="{
                          height: countRowspanHeight(item, tdItem, index, tdItemIndex),
        ...
        <template v-if="list.length">
          ...
          <view
            class="fixed-left"
            v-if="columnsFixedLeft[0] || (selection == 'mulit' && fixedCheckbox)"
          >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <slot
                    :row="item"
                    v-if="slotCols.indexOf(columnsFixedLeft[0] && columnsFixedLeft[0].key) > -1"
                  ></slot>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <slot
                    :row="item"
                    v-if="slotCols.indexOf(columnsFixedRight[0] && columnsFixedRight[0].key) > -1"
                  ></slot>
    ...
    methods: {
      ...
      countColspanWidth(item, tdItem, index, tdItemIndex, iswrap = false) {
        ...
        if (moreThanOne > 1) {
          ...
          //不跨列
          let tmp =
            this.columns[tdItemIndex].$width && parseInt(this.columns[tdItemIndex].$width)
      ...
      countRowspanHeight(item, tdItem, index, tdItemIndex) {
        ...
        let t = moreThanOne
          ? this.spanMethod(item, tdItem, index, tdItemIndex)['rowspan'] * this.tdHeight + 'px'
          : this.tdHeight + 'px';
        return t;
      },
      /*
