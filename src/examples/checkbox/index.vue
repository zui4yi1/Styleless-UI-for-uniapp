<template>
  <s-page title="sl-checkbox" className="plr-l">
    <Brief
      desc="主要属性value和list, 其它还有方向direction, 形状type, 尺寸size等（与sl-radio基本属性一样, 除了value是数组及多了type形状）"
    />
    ...
    <Tip
      title="文字换行, 使用垂直模式"
      desc="direction=y. 文字太多时, 会自动换行。但因换行后, 行间距不容易控制, 故请直接使用垂直模式"
      className="mt-d"
    />
    <sl-checkbox v-model:value="val" :list="list2" direction="y" className="mt-d" />
  </s-page>
</template>
